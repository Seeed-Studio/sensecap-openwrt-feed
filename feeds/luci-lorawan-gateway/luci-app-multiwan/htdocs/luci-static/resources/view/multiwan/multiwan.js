...
return view.extend({
	...
	render: function () {
		...
		m = new form.Map('mwan3', _('MultiWAN Interfaces'),
			_('MultiWAN Interfaces are profiles attaching a metric and weight to an MWAN interface.') + '<br />' +
			_('Names may contain characters A-Z, a-z, 0-9, _ and no spaces, and may not share the same name as configured interfaces.') + '<br />' +
            _('Metric defines the priority of the interface, lower metrics have higher priority.') + '<br />' +
            _('Weight defines the relative amount of traffic to be sent over this interface compared to other interfaces with the same metric.'));
