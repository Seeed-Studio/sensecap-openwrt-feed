/* OTA_DOWNLOAD_VERSION_1766217005 */
'use strict';
'require view';
'require dom';
'require form';
...
return view.extend({
	...
	pollOTAProgress: function (btn) {
		...
		var checkProgress = function () {
			// Read static JSON file - no CGI, no rpcd, no timeout!
			var xhr = new XMLHttpRequest();
			xhr.open('GET', '/ota-progress.json?_=' + Date.now(), true);

			xhr.onload = function () {
				if (xhr.status === 200) {
					try {
						var data = JSON.parse(xhr.responseText);
						var progressDiv = document.getElementById('ota-progress');
						...
						if (data.status === 'complete') {
							ui.showModal(_('Download Complete'), [
								E('p', _('Firmware downloaded successfully. Click Upgrade to apply.')),
								E('div', { 'class': 'right' }, [
									E('button', {
										'class': 'btn',
										'click': function () {
											fs.exec('/bin/ota', ['cancel']);
											ui.hideModal();
										}
									}, [_('Cancel')]),
									' ',
									E('button', {
										'class': 'btn cbi-button-action important',
										'click': ui.createHandlerFn(self, 'handleOTAUpgradeConfirm', btn)
									}, [_('Upgrade')])
								])
							]);
	...
	handleOTAUpgradeConfirm: function (btn, ev) {
		ui.showModal(_('Upgrading firmware...'), [
			E('p', { 'class': 'spinning' }, _('Extracting and applying firmware. DO NOT POWER OFF!'))
		]);

		return fs.exec('/bin/ota', ['upgrade'])
			.then(function (res) {
				if (res.code != 0) {
					ui.hideModal();
					ui.addNotification(null, E('p', _('Upgrade failed: ') + (res.stderr || 'Unknown error')));
					return Promise.reject('Upgrade failed');
				}

				ui.showModal(_('Rebooting...'), [
					E('p', { 'class': 'spinning' }, _('System is rebooting. Please wait...'))
				]);

				ui.awaitReconnect('192.168.254.19');
			})
			.catch(function (e) {
				ui.hideModal();
